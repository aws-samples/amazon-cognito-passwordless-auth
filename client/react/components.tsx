...
export const Passwordless = ({
  ...
  const [showOptionsForUser, setShowOptionsForUser] = useState<
    "NONE" | "LAST_USER" | "NEW_USER"
  >();
  ...
  if (showOptionsForUser === undefined && lastUser) {
    setShowOptionsForUser("LAST_USER");
  }
  const showUser =
    showOptionsForUser === "LAST_USER" || showOptionsForUser === "NEW_USER";
  const user: typeof lastUser = newUsername
    ? {
        email: newUsername,
        username: newUsername,
        useFido: "YES", // presume the user might want to use FIDO2
      }
    : lastUser;

  return (
    <FlexContainer brand={brand}>
      {signInStatus === "NOT_SIGNED_IN" && showUser && user && (
        <div>
          <div>
            ...
            <p className="passwordless-flex passwordless-flex-vertical-buttons">
              ...
              <button
                className={`passwordless-button passwordless-button-sign-in ${
                  user.useFido === "YES" ? "passwordless-button-outlined" : ""
                }`}
                onClick={() =>
                  requestSignInLink({
                    username: user.username,
          ...
          <div
            ...
            onClick={() => setShowOptionsForUser("NONE")}
          >
      ...
      {signInStatus === "NOT_SIGNED_IN" && (!showUser || !user) && (
        <>
          ...
          >
            <button className="passwordless-button" disabled={busy}>
              <div className="passwordless-flex">
                <div className="passwordless-svg-icon-container">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
